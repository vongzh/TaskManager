...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public virtual bool Add(DbConn conn, tb_node_model model)
        {
            ...
            int rev = conn.ExecuteSql(@"insert into tb_node(nodename,nodecreatetime,nodeip,nodelastupdatetime,ifcheckstate)
										   values(@nodename,@nodecreatetime,@nodeip,@nodelastupdatetime,@ifcheckstate)", Par);
            return rev == 1;

        }

        public virtual bool Edit(DbConn conn, tb_node_model model)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>()
            {
            ...
            int rev = conn.ExecuteSql("update tb_node set nodename=@nodename,nodecreatetime=@nodecreatetime,nodeip=@nodeip,nodelastupdatetime=@nodelastupdatetime,ifcheckstate=@ifcheckstate where id=@id", Par);
            return rev == 1;
        ...
        public virtual tb_node_model Get(DbConn conn, int id)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>();
            Par.Add(new ProcedureParameter("@id", id));
