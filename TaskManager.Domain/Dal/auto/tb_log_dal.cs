...
namespace TaskManager.Domain.Dal
{
	...
	public partial class tb_log_dal
    {
        public virtual bool Add(DbConn conn, tb_log_model model)
        {
            ...
            int rev = conn.ExecuteSql(@"insert into tb_log(msg,logtype,logcreatetime,taskid,nodeid)
										   values(@msg,@logtype,@logcreatetime,@taskid,@nodeid)", Par);
            return rev == 1;

        }

        public virtual bool Edit(DbConn conn, tb_log_model model)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>()
            {
            ...
            int rev = conn.ExecuteSql("update tb_log set msg=@msg,logtype=@logtype,logcreatetime=@logcreatetime,taskid=@taskid,nodeid=@nodeid where id=@id", Par);
            return rev == 1;
        ...
        public virtual tb_log_model Get(DbConn conn, int id)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>();
            Par.Add(new ProcedureParameter("@id", id));
