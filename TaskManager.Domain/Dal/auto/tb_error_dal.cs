...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public virtual bool Add(DbConn conn, tb_error_model model)
        {
            ...
            int rev = conn.ExecuteSql(@"insert into tb_error(msg,errortype,errorcreatetime,taskid,nodeid)
										   values(@msg,@errortype,@errorcreatetime,@taskid,@nodeid)", Par);
            return rev == 1;

        }

        public virtual bool Edit(DbConn conn, tb_error_model model)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>()
            {
            ...
            int rev = conn.ExecuteSql("update tb_error set msg=@msg,errortype=@errortype,errorcreatetime=@errorcreatetime,taskid=@taskid,nodeid=@nodeid where id=@id", Par);
            return rev == 1;
        ...
        public virtual tb_error_model Get(DbConn conn, int id)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>();
            Par.Add(new ProcedureParameter("@id", id));
