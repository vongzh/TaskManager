...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public virtual bool Add(DbConn conn, tb_task_model model)
        {
            ...
            int rev = conn.ExecuteSql(@"insert into tb_task(taskname,categoryid,nodeid,taskcreatetime,taskupdatetime,tasklaststarttime,tasklastendtime,tasklasterrortime,taskerrorcount,taskruncount,taskcreateuserid,taskstate,taskversion,taskappconfigjson,taskcron,taskmainclassnamespace,taskremark)
        ...
        public virtual bool Edit(DbConn conn, tb_task_model model)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>()
            {
        ...
        public virtual tb_task_model Get(DbConn conn, int id)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>();
            Par.Add(new ProcedureParameter("@id", id));
            StringBuilder stringSql = new StringBuilder();
            stringSql.Append(@"select s.* from tb_task s where s.id=@id");
            int rev = conn.ExecuteSql(stringSql.ToString(), Par);
