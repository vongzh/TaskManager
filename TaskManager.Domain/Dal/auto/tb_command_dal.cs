...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public virtual bool Add(DbConn conn, tb_command_model model)
        {
            ...
            int rev = conn.ExecuteSql(@"insert into tb_command(command,commandname,commandstate,taskid,nodeid,commandcreatetime)
										   values(@command,@commandname,@commandstate,@taskid,@nodeid,@commandcreatetime)", Par);
            return rev == 1;

        }

        public virtual bool Edit(DbConn conn, tb_command_model model)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>()
            {
            ...
            int rev = conn.ExecuteSql("update tb_command set command=@command,commandname=@commandname,commandstate=@commandstate,taskid=@taskid,nodeid=@nodeid,commandcreatetime=@commandcreatetime where id=@id", Par);
            return rev == 1;
        ...
        public virtual tb_command_model Get(DbConn conn, int id)
        {
            List<ProcedureParameter> Par = new List<ProcedureParameter>();
            Par.Add(new ProcedureParameter("@id", id));
