...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public List<int> GetTaskIDsByState(DbConn conn, int taskstate,int nodeid)
        {
        ...
        public List<tb_task_model> GetLongRunningTaskIDs(DbConn conn, int maxrunningtimeseconds)
        {
            return SqlHelper.Visit(ps =>
            {
        ...
        public List<tb_tasklist_model> GetList(DbConn conn, string taskid, string keyword, string CStime, string CEtime, int categoryid, int nodeid ,int userid, int state , int pagesize, int pageindex, out int count)
        {
            int _count = 0;
            ...
            DataSet dsList = SqlHelper.Visit<DataSet>(ps =>
            {
                ...
                _count = Convert.ToInt32(conn.ExecuteScalar("select count(1) from tb_task T where 1=1 " + sqlwhere, ps.ToParameters()));
                DataSet ds = new DataSet();
        ...
        public List<tb_task_model> GetListAll(DbConn conn)
        {
        ...
        public tb_task_model GetOneTask(DbConn conn, int taskid)
        {
            return SqlHelper.Visit(ps =>
            {
                ps.Add("id", taskid);
        ...
        public int AddTask(DbConn conn, tb_task_model model)
        {
            return SqlHelper.Visit<int>(ps =>
            {
                ...
                int rev = Convert.ToInt32(conn.ExecuteScalar(@"insert into tb_task(taskname,categoryid,nodeid,taskcreatetime,taskruncount,taskcreateuserid,taskstate,taskversion,taskappconfigjson,taskcron,taskmainclassnamespace,taskremark,taskmainclassdllfilename)
        ...
        public int UpdateTask(DbConn conn, tb_task_model model)
        {
            return SqlHelper.Visit<int>(ps =>
            {
                ps.Add("@id", model.id);
        ...
        public int CheckTaskState(DbConn conn, int id)
        {
            return SqlHelper.Visit(ps =>
            {
                ps.Add("id", id);
                string sql = "select taskstate from tb_task where id=@id";
                int i = Convert.ToInt32(conn.ExecuteScalar(sql, ps.ToParameters()));
                return i;
            });
        }

        public int ChangeTaskState(DbConn conn, int id, int state)
        {
            return SqlHelper.Visit(ps =>
            {
                ps.Add("taskstate", state);
        ...
        public int UpdateTaskState(DbConn conn, int taskid, int taskstate)
        {
            return SqlHelper.Visit(ps =>
            {
                ps.Add("@taskstate", taskstate);
        ...
        public bool DeleteOneTask(DbConn conn, int id)
        {
