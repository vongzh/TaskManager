...
namespace TaskManager.Domain.Dal
{
    ...
    {
        public int Add2(DbConn conn, tb_error_model model)
        {
        ...
        public List<tb_senderror_model> GetErrors(DbConn conn, int lastlogid)
        {
            return SqlHelper.Visit(ps =>
            {
        ...
        public List<tb_errorinfo_model> GetList(DbConn conn, string keyword, int id, string cstime, string cetime, int errortype, int taskid, int nodeid, int pagesize, int pageindex, out int count)
        {
            ...
            DataSet dsList = SqlHelper.Visit<DataSet>(ps =>
            {
                ...
                _count = Convert.ToInt32(conn.ExecuteScalar("select count(1) from tb_error E where 1=1 " + sqlwhere, ps.ToParameters()));
                DataSet ds = new DataSet();
