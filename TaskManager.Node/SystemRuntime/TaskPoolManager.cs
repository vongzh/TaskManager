    ...
    public class TaskPoolManager:IDisposable
    {
        ...
        public bool Add(string taskid, NodeTaskRuntimeInfo taskruntimeinfo)
        {
            lock (_locktag)
            {
                ...
                {
                    JobDetailImpl jobDetail = new JobDetailImpl(taskruntimeinfo.TaskModel.id.ToString(), taskruntimeinfo.TaskModel.categoryid.ToString(), typeof(TaskJob));// 任务名，任务组，任务执行类  
        ...
        public bool Remove(string taskid)
        {
            lock (_locktag)
            {
                if (TaskRuntimePool.ContainsKey(taskid))
                {
                    ...
                    TriggerKey triggerKey = new TriggerKey(taskruntimeinfo.TaskModel.id.ToString(), taskruntimeinfo.TaskModel.categoryid.ToString());
                    _sched.PauseTrigger(triggerKey);// 停止触发器  
                    _sched.UnscheduleJob(triggerKey);// 移除触发器  

                    JobKey jobKey = new JobKey(taskruntimeinfo.TaskModel.id.ToString(), taskruntimeinfo.TaskModel.categoryid.ToString());
                    _sched.DeleteJob(jobKey);// 删除任务
